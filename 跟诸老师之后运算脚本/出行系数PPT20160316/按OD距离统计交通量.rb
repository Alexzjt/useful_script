...
outfile = File.new("D:\\zjt\\实验结果\\20160317\\#{date.to_s}出行系数_#{xiajie}_#{shangjie}_#{changdu}_ETC_MTC.csv","w")
output_str = ""
jishuqi=0
temp = 0
...
file_array = []
Find.find("D:\\zjt\\2015年数据\\201510\\") do |filename|
	file_array.push filename
end
Find.find("D:\\zjt\\2015年数据\\201511\\") do |filename|
	file_array.push filename
end
Find.find("D:\\zjt\\2015年数据\\201512\\") do |filename|
	file_array.push filename
end
odhash = Hash.new(0)
odfile.each do |line|
...
file_array.each do |file|
	...
	while jishuqi<shangjie/changdu
		output_str+=",#{etc_count[jishuqi]},#{mtc_count[jishuqi]}"
		jishuqi+=1
	end
	outfile.puts(output_str)
	date+=1
